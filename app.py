...
def initialize_tools(config: Dict[str, Any], llm: BaseLanguageModel) -> List[BaseTool]:
    ...
    for tool_name in config.get('default_tools', ['search', 'calculator']):
        try:
            if tool_name == 'search':
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Run LangChain Chat Agent')
    ...
    try:
        ...
        # Create agent using from_llm_and_tools
